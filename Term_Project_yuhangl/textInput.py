# this class file is copied from
...
class TextInput:
    ...
    blinking cursor that can be moved using the arrow-keys. Delete, home and 
    end work as well.
    """
    def __init__(
            self,
        ...
        :param font_family: name or list of names for font 
         (see pygame.font.match_font for precise format)
        :param font_size:  Size of font in pixels
        :param antialias: Determines if antialias is applied to font 
         (uses more processing power)
        :param text_color: Color of text (duh)
        :param cursor_color: Color of cursor
        :param repeat_keys_initial_ms: Time in ms before keys are repeated 
         when held
        :param repeat_keys_interval_ms: Interval between key press repetition 
         when held
        ...
        for key in self.keyrepeat_counters:
            ...
            if self.keyrepeat_counters[key][0] >= \
                ...
                pygame.event.post(pygame.event.Event(pl.KEYDOWN, \
                                        key=event_key, unicode=event_unicode))
